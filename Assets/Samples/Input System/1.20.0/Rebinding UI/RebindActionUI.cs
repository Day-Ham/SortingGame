...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public void SwapBinding(RebindActionUI other, string originalPath)
        {
            if (this == other)
            ...
            otherAction.ApplyBindingOverride(otherBindingIndex, originalPath);
            
            UpdateBindingDisplay();
            other.UpdateBindingDisplay();

            m_RebindDuplicatePromp.SetActive(false);
            m_RebindOverlay.SetActive(false);

            m_ConfirmSwapButton.onClick.RemoveListener(ConfirmSwapBinding);
            m_CancelSwapButton.onClick.RemoveListener(CancelSwapBinding);
        }

        private void ConfirmSwapBinding()
        {
            Debug.Log($"m_DuplicateBinding = {m_DuplicateBinding}");

            if (m_DuplicateBinding == null)
            {
                Debug.LogError("Duplicate binding UI is NULL!");
                return;
            }

            SwapBinding(m_DuplicateBinding, m_PendingOriginalPath);
        }

        private void CancelSwapBinding()
        {
            m_PendingAction.RemoveBindingOverride(m_PendingBindingIndex);
            m_RebindDuplicatePromp.SetActive(false);
            ...
            PerformInteractiveRebind(m_PendingAction, m_PendingBindingIndex, m_PendingAllCompositeParts);
        }
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            string originalPath = action.bindings[bindingIndex].effectivePath;
            ...
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (CheckDuplicateBindings(action, bindingIndex, out RebindActionUI duplicateBinding, allCompositeParts))
                        {
                            CleanUp();

                            ShowDuplicatePromp(action, bindingIndex, duplicateBinding, originalPath, allCompositeParts);
                            return;   
                        }
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, out RebindActionUI duplicateBinding, bool allCompositeParts = false)
        {
            duplicateBinding = null;

            InputBinding newBinding = action.bindings[bindingIndex];

            foreach (RebindActionUI ui in s_RebindActionUIs)
            {
                // Skip the UI that is currently being rebound.
                if (ui.ResolveActionAndBinding(out var otherAction, out var otherBindingIndex))
                {
                    InputBinding otherBinding = otherAction.bindings[otherBindingIndex];

                    // Don't compare the binding against itself.
                    if (otherAction == action && otherBinding.id == newBinding.id)
                        continue;

                    // Check if the other UI is using the same effective binding.
                    if (otherBinding.effectivePath == newBinding.effectivePath)
                    {
                        duplicateBinding = ui;

                        Debug.Log($"Duplicate binding found: {newBinding.effectivePath} " + $"is already used by {otherAction.name}");
            ...
            // Check for duplicate bindings between parts of the same composite.
            if (allCompositeParts)
            {
        ...
        private void ShowDuplicatePromp(InputAction action, int bindingIndex, RebindActionUI duplicateBinding, string originalPath, bool allCompositeParts = false)
        {
            m_PendingAction = action;
            m_PendingBindingIndex = bindingIndex;
            m_PendingOriginalPath = originalPath;
            m_PendingAllCompositeParts = allCompositeParts;

            m_DuplicateBinding = duplicateBinding;

            m_RebindDuplicatePromp.SetActive(true);

            m_ConfirmSwapButton.onClick.AddListener(ConfirmSwapBinding);
        ...
        private InputAction m_PendingAction;
        private int m_PendingBindingIndex;
        private bool m_PendingAllCompositeParts;
        private string m_PendingOriginalPath;
        ...
        [Tooltip("Optional UI that will be shown when trying to rebind with a key that is already in use.")]
        [SerializeField]
        private GameObject m_RebindDuplicatePromp;
        
        [Tooltip("Optional UI that will be shown when trying to rebind with a key that is already in use.")]
        [SerializeField]
        private Button m_ConfirmSwapButton;

        [Tooltip("Optional UI that will be shown when trying to rebind with a key that is already in use.")]
        [SerializeField]
        private Button m_CancelSwapButton;
